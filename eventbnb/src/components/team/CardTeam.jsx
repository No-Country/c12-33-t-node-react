...
const CardTeam = ({color, name, rol, image, socialMedia, href}) => {
    return(
        <div className={`w-80 flex flex-col items-center gap-y-6 border-t-4 border-t-${color} rounded-b-lg px-6 bg-black/5 py-10`}>
            ...
            <Link href={`./about/${href}`} className={`self-end flex items-center gap-x-2 font-semibold group text-${color} underline`}><span className="group-hover:scale-[1.05]">Saber mas</span><AiOutlineArrowRight className="group-hover:translate-x-2"></AiOutlineArrowRight></Link>
        </div>
    )
} 
