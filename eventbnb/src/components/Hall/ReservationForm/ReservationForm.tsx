...
const ReservationForm = () => {
  ...
  const { eventHall, disabled, formattedDateReservation } = useContext(EventHallContext) as IEventHallProvider;
  ...
  return (
    <div className="p-6 mt-10 border rounded-xl shadow-xl sticky top-56">
      <div className="flex justify-between pb-5">
        <div>
          <span className="text-xl font-medium">${eventHall.precio}</span> <span>noche</span>
        </div>
        <p className="flex gap-1.5 items-center">
          ...
          · <span className="text-sm text-gray-500">{eventHall.reviews.length} reseñas</span>
        </p>
      </div>
      <div>
        <span>Reservar para: <b>{formattedDateReservation}</b></span>
      </div>
      <div className="pt-5">
        <button 
        onClick={handleClick}
        className={` ${disabled ? ' hover:cursor-not-allowed' : 'hover:cursor-pointer hover:bg-rose-400'} bg-rose-500 rounded-lg text-white font-medium py-3 px-3 w-full`}>
          Reserva
        </button>
      </div>
