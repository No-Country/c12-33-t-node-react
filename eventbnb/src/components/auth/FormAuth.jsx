...
const SignIn = ({hidden, setHidden}) => {
    ...
    function handleChange(e){
        ...
        setErrors(validationErrors({...localLogin, [e.target.name]: e.target.value}))
      }
    ...
    return(
            <div  className={`${hidden ? 'w-full h-full flex items-center justify-center fixed top-0 left-0 z-40 py-4 overflow-x-hidden overflow-y-auto  bg-black/40' : 'hidden'}`}>
                <div className='h-full shadow-xl gap-y-4 bg-white pb-4 text-black w-full xl:w-[40%] mx-auto rounded-xl flex flex-col'>
                    ...
                    <div className='px-4 flex items-center justify-between w-full rounded-t-lg py-2 border-b'>
                        <AiOutlineClose className='text-4xl text-black p-2 rounded-full hover:cursor-pointer hover:bg-gray-600/5' onClick={() => setHidden(false)}></AiOutlineClose>
                        <p className='mx-auto font-semibold'>Iniciá sesión o registrate</p>
                    </div>
                    <div className='overflow-y-auto px-6 flex flex-col gap-y-6'>

                        <h2 className='my-2 text-xl font-semibold'>Te damos la bienvenida a Eventbnb</h2>
                        <form className='' onSubmit={handleSubmit} action="">
                            <div className="relative z-0 w-full group mt-6">
                                <input value={localLogin.email} type="email" name="email" onChange={handleChange} id="floating_email" className="block w-full text-sm text-gray-900 bg-transparent border rounded-lg py-4 px-2 border-gray-300  focus:outline-none focus:ring-0 focus:border-black peer" placeholder="" required />
                                <label className="peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-2 -z-10 origin-[0] peer-focus:left-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-10">Correo electrónico</label>
                            </div>
                            <div className="relative z-0 w-full group mt-10">
                                <input value={localLogin.password} type="password" name="password" onChange={handleChange} id="floating_password" className="border-t-transparent border rounded-lg w-full py-4 px-2 block text-sm text-gray-900 bg-transparent focus:outline-none focus:ring-0 focus:border-black peer" placeholder="" required />
                                <label  className="peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-2 -z-10 origin-[0] peer-focus:left-0  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-10">Contraseña</label>
                            </div>
                            <p className='text-[12px] mt-2'>
                            Vamos a llamarte o enviarte un mensaje para confirmar el número. Se aplican tarifas estándar para mensajes y uso de datos. <Link className='underline font-semibold' href="">Política de privacidad</Link>
                            </p>
                            <button className="w-full rounded-lg bg-gradient-to-r from-pink-700 via-pink-600 to-pink-700 text-white font-semibold py-[12px] mt-4" type="submit">Continuar</button>
                        </form>
                        {
                            errors.email && 
                        ...
                        </div>
                        {
                            ...
                            <div className='flex flex-col gap-y-4 '>
                                ...
                                >
                                    ...
                                    <span className='self-center mx-auto'>Registrate con Google</span>
                                </button>
                            </div> :
                            <button
                            type='button'
