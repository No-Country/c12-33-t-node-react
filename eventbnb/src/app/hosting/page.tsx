        ...
        <section className="text-black bg-white w-full px-6 md:px-24 flex flex-col gap-y-6 md:gap-y-16">
            ...
            <div className="flex flex-col gap-y-6">
                <div className="flex flex-col md:flex-row items-center justify-between">
                    ...
                    <Link className="text-sm md:text-lg underline font-semibold" href='/hosting/reservations'>Todas las reservas {'(${cantidad de reservas})'}</Link>
                </div>
                <div className="flex flex-col md:flex-row items-center gap-x-4 flex-wrap gap-y-4">
                    <button className="border rounded-full px-2 py-1 hover:border-black">Hacen el check-out pronto {'(${cantidad de reservas})'}</button>
                    <button className="border rounded-full px-2 py-1 hover:border-black">Estadías en curso {'(${cantidad de reservas})'}</button>
                    <button className="border rounded-full px-2 py-1 hover:border-black">Llegan pronto {'(${cantidad de reservas})'}</button>
                    <button className="border rounded-full px-2 py-1 hover:border-black">Próximas {'(${cantidad de reservas})'}</button>
                    <button className="border rounded-full px-2 py-1 hover:border-black">Evaluación pendiente {'(${cantidad de reservas})'}</button>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-y-8">
                ...
                <div className="flex flex-col md:flex-row items-center gap-x-6 gap-y-4">
                    <div className='flex items-start gap-x-4 border rounded-lg w-full md:w-1/2 xl:w-[40%] px-4 py-2'>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16" viewBox="0 0 32 32" aria-hidden="true" role="presentation" focusable="false" ><path d="M22 5a6 6 0 0 1 3.64 10.77A9 9 0 0 1 31 23.74V24h-2a7 7 0 0 0-6-6.93v-2.2A4 4 0 0 0 22 7a4 4 0 0 0-3.68 5.57A5 5 0 0 1 21 17a4.99 4.99 0 0 1-1.6 3.67 9 9 0 0 1 5.6 8.06V29h-2a7 7 0 0 0-6-6.93v-2.24a3 3 0 1 0-2 0v2.24a7 7 0 0 0-6 6.69V29H7a9 9 0 0 1 5.6-8.34 5 5 0 0 1 1.08-8.09A4 4 0 1 0 9 14.87v2.2a7 7 0 0 0-6 6.69V24H1a9 9 0 0 1 5.36-8.23A6 6 0 1 1 15.92 10h.16A6 6 0 0 1 22 5z"></path></svg>
                        <div>
                            <h4 className="font-semibold text-lg">Sumate al club de anfitriones de tu zona</h4>
                            <p className="text-slate-600">Conocé a otros anfitriones y miembros de la comunidad con quienes colaborar e intercambiar consejos.</p>
                        </div>
                    </div>
                    <div className='flex items-start gap-x-4 border rounded-lg w-full md:w-1/2 xl:w-[40%] px-4 py-2'>
